...
class BaseModel:
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        cls.__slots__ = getattr(cls, "__slots__", []) + BaseModel.__slots__

    def __init__(self, **attributes):
        """
        Cria uma instância de um modelo que usa **BaseModel**
        ...
        if db is None:
            raise AttributeError("Specify the database using __db__ when structuring the model")
        ...
        if not attributes.get("instance") is False:
            ...
            for attr, value in attributes.items():
                if attr not in ann:
                    raise AttributeError(f'{type(self).__name__} does not have "{attr}" attribute!')          
                      
                typ: type = ann[attr]
                try:
                    count_db: None
                    attrs[attr] = typ(value)
                    # if attr != idname or value is not None:
                    # else:
                    #     attrs[attr] = RedisConnect.count # o ID será gerado somente na hora de adicionar no banco de dados
                except ValueError:
                    received = value if value else count_db
                    raise ValueError(f"{attr} expected a possible {typ.__name__} value, but received a {type(received).__name__} ({received}) value!")
            ...
            for attr in ann:
                if not str(attr).startswith("__") and attr not in attrs:
                    ...
                    if not hasattr(name, attr):
                        raise AttributeError(f"{attr} must receive a value!")
